...
func DefaultReadBlockIndex(ctx context.Context, in *BlockIndex, db *gorm.DB) (*BlockIndex, error) {
	...
	if db, err = gorm1.ApplyFieldSelection(ctx, db, nil, &BlockIndexORM{}); err != nil {
		return nil, err
	}
...
func DefaultListBlockIndex(ctx context.Context, db *gorm.DB) ([]*BlockIndex, error) {
	...
	db, err = gorm1.ApplyCollectionOperators(ctx, db, &BlockIndexORM{}, &BlockIndex{}, nil, nil, nil, nil)
	if err != nil {
		return nil, err
	}
